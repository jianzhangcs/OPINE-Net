...
def test():
    args = option_multiple.get_args_test()
    ...
    n_output = 1089
    # our model
    model = models.get_ISTANet_Multiple(layer_num, n_input, share_flag)
    ...
    model_dir = "OPINE_Net_Multiple_share_%d_phase_%d_group_%d_%s_Binary_Norm_%.2f_Single_%.4f" % (share_flag, layer_num, group_num, ''.join([k for k in CS_ratio]), phi_weight, learning_rate)
    ...
    with torch.no_grad():
        for img_no in range(ImgNum):
            ...
            # sio.savemat('OPINE_Net_phi_%d.mat' % int(CS_ratio[phi_index]), {'Phi': phi_value})
            print("Run time for image %2d is %.4fs, PSNR is %.2f" % (img_no, (end - start), rec_PSNR))
            Img_rec_yuv[:,:,0] = X_rec*255

            im_rec_rgb = cv2.cvtColor(Img_rec_yuv, cv2.COLOR_YCrCb2BGR)
            im_rec_rgb = np.clip(im_rec_rgb, 0, 255).astype(np.uint8)
            cv2.imwrite("%s_OPINENet_Orth_Single_ratio_%d_epoch_%d_PSNR_%.2f_SSIM_%.4f.png" % (imgName, int(CS_ratio[phi_index]), epoch_num, rec_PSNR, rec_SSIM), im_rec_rgb)
            del y_pred
    ...
    print(test_dir)
    print('\n')
    output_data = "CS ratio is %d, Avg PSNR/SSIM is %.2f/%.4f, cpkt NO. is %d, phi index is %d \n" % (int(CS_ratio[phi_index]), np.mean(PSNR_All), np.mean(SSIM_All), epoch_num, phi_index)
    print(output_data)
